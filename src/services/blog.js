...
// READ
export const getBlogs = ({page, limit, order, name, category, ...query}) => new Promise(async (resolve, reject) => {
    try{
...
// CREATE
export const createNewBlog = (body, fileData) => new Promise(async (resolve, reject) => {
    try{
        const response = await db.Blog.findOrCreate({
            where: {titleBlog: body?.titleBlog},
            defaults: {
                ...body,
                id: generated(),
                firstImage: fileData?.path,
