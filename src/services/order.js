...
// READ
export const getOrders = ({ page, limit, order, name, ...query }) =>
...
// UPDATE
export const updateOrder = ({ bid, ...body }) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        ...
        mes:
          response[0] > 0
            ? `${response[0]} order updated`
            : "Cannot update order/Order id not found",
      });
    } catch (err) {
      reject(err);
    }
      ...
      resolve({
        ...
        mes:
          response > 0
            ...
            : "Cannot delete order/Order id not found",
      });
    } catch (err) {
      reject(err);
    }
